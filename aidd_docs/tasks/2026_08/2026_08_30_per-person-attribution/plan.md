...
| `scanHarness` gains the paths that proved each member. | It reports capabilities today, and authorship needs to know which files earned them. The addition is a second field on `HarnessScan`; the existing one is untouched, so nothing that reads capabilities changes. |
...
| A row prints its commit count and its demonstrated reading even when it carries no level. | The count is what separates "nothing to measure" from "measured and low", and it is the whole reason a reader can tell those apart. The demonstrated reading is rendered per row on the same terms as the repository's: never without the share that earned it. |
