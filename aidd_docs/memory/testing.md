...
| `harness.command` (`runHarness`) | argv parsing, tiers, scopes, unread entries and JSON/human rendering | `CommandIo`, `ClaudeHarnessAdapter(emptyMachineDirectory())`, `InfiniteTokenEncoderTestAdapter` |
| `tests/cli/process-contract.test.ts` | that `main.ts` and the built `dist/cli.js` deliver that taxonomy to a real shell, that the wired collector reaches the pipeline through it, and that colour follows the channel rather than the report | none — the process is spawned, nothing is faked |
