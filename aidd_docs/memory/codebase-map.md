...
- `tests/`: only the suites that exercise no single file — the conformance of `aidd.yml`, the conformance of the three status vocabularies, `cli/process-contract.test.ts` (the exit-code and stream contract, observed by spawning the built `dist/cli.js`), `cli/reference-profiles.test.ts` (the four reference profiles assessed through `runAssess`). Every other suite sits beside its subject in `src/`; see `testing.md`.
