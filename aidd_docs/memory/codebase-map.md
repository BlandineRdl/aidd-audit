...
**Status: `levels/`, `profiles/`, `aidd_docs/`, `aidd.yml`, `src/` and `tests/` all exist as frozen in `aidd_docs/INSTALL.md`, under its "Folder structure" section.** No production `EvidenceCollector` exists, so no reference profile reaches its expected level yet, and the acceptance suite that would prove it is still owed — see `cli.md` and `testing.md`.
