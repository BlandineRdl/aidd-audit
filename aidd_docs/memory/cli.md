...
- Exit codes: `0` success (including `proven: null` — a valid result, not an error); `2` the caller's fault (`UsageError`, `InvalidMaturityModelError`); `1` ours (`UnrenderableReportError` included). A caller that only checks non-zero cannot tell "you typed it wrong" from "the tool broke"; this taxonomy lets it.
