...
- No collection timeout is set. `CollectorContext.signal` makes honouring the budget a collector's own duty, and the production collector set is empty today — a timeout constant now would guard nothing measured. `runAssess` still owns an `AbortController`, aborted in `finally`, so the seam exists and the budget lands with the first real collector.
- **No production `EvidenceCollector` exists yet, and none of the four reference profiles reaches its expected level as a result.** `collectors` in `assess.command.ts` is `[]`; every axis resolves `UNKNOWN`, so every `aidd-audit assess <profile>` reports `proven: null`, whatever the profile's expected level in `testing.md`. This is the accepted scope limit of this feature, not a defect — the fixture bundle and live-repository adapters plug into the same `collectors` field with no signature change once they land.
...
- A third state sits above both: no collector ran at all, so nothing was ever looked at. `human.renderer.ts` names it (keyed on `report.provenance.length === 0`) before a reader reaches the blocking list, so "we looked and found nothing" is never mistaken for "we never looked" — the only report the empty `collectors` array in `assess.command.ts` can currently produce.
...
`tests/cli/process-contract.test.ts` pins `0` and `2` through the built binary. **`1` is unreachable today and therefore untested**: `collectors` in `assess.command.ts` is `[]`, so no observed value exists, no collector can raise `InvalidObservationError`, and no non-finite number can reach a report — a non-finite rank is already refused at load. Delete the `UnrenderableReportError` path and nothing observable changes. It is owed a test the day a production collector lands, and `collectors` has to become injectable before one can be written. Recorded as owed rather than counted as covered.
